$(document).ready(function () {
	...
	$("#board tr td").click(function () {
		if ($(this).text() == "" && play) {
			if ((move % 2) == 1) {
				$(this).append("X");
				$(this).css('color', "rgb(139,255,211)");
			...
			if (checkForWinner() != -1 && checkForWinner() != "") {
				if (checkForWinner() == "X") {
					$('body').append('<div class="winner"><span>Ganador</span>X</div><button onclick="location.reload();" id="reload">Jugar de nuevo</button>');
					$('.winner').css('background-color', 'rgb(139,255,211)');
					$('#reload').css('color','rgb(139,255,211)');
				} else {
					$('body').append('<div class="winner"><span>Ganador</span>O</div><button onclick="location.reload();" id="reload">Jugar de nuevo</button>');
					$('.winner').css('background-color', 'rgb(255,0,0)');
					$('#reload').css('color','rgb(255,0,0)');
				}
				play = false;
			}
		}
